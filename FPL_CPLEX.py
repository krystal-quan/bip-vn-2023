...
def update(player_list):
    ...
    # T = [f"gws\gw{i + 1}.csv" for i in range(nT)]
    ...
    P = [1 if PD[i] == 1 else 2 if PM[i] == 2 else 3 if PF[i] == 3 else 4 for i in range(nP)]
    PC = [[0 for i in range(MC)] for i in range(nC)]
...
def run_cplex(week):
    ...
    m.add_constraint(m.sum(y_p[i] for i in P) == E)
    ...
    m.add_constraint(m.sum(f_p[i] for i in P)== 1)
    m.add_constraint(m.sum(h_p[i] for i in P)== 1)

    # m.add_constraint(m.sum(g_pl[i, j] for i in P for j in L) <= 1)
    if (week == 1): 
        m.add_constraint((m.sum(CB_pt[i] * x_p[i] for i in P) - BS + v) == 0)
    
    if (week >= 2):
        m.add_constraint((v_t[week - 1] + m.sum((CS_pt[i] * u_p[i] for i in P) - 
                                               (CB_pt[i] * e_p[i] for i in P)) - v) == 0)
        m.add_constraints((x_pt[i][week - 1] + e_p[i] - u_p[i] - x_p[i]) == 0 for i in P)
        m.add_constraint(m.sum(u_p[i] for i in P) <= E)
        m.add_constraint(m.sum(e_p[i] for i in P) <= E)
    m.add_constraints((e_p[i] + u_p[i]) <= 1 for i in P)
    if (week == 2): q = Q
    # m.add_constraint(A * (Qp - q) >= al)
    m.add_constraint(q >= Q)
    m.add_constraint(q - Qp <= 0)
    pointNonCaptain = m.sum((P_pt[i] * y_p[i]) for i in P)
    pointCaptain = m.sum((P_pt[i] * f_p[i]) for i in P) * 2
    pointVice = m.sum((P_pt[i] * h_p[i]) for i in P) * 1.5
    # (2 if m.sum((P_pt[i] * f_p[i]) for i in P) == 0 else 1)
    deductedPoints = R * al
    m.maximize(pointNonCaptain + pointCaptain + pointVice - deductedPoints)
